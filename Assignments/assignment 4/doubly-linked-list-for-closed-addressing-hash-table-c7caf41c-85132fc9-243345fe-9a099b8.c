...
{
    ListNode *keynode=HashSearch(*Q1Ptr, key);

    if (keynode!=NULL){
        return 0;
    }

    //create new node as keynode
    keynode = (ListNode*)malloc(sizeof(ListNode));
    keynode->key = key;
   
    int index = Hash(key,Q1Ptr->hSize);

    keynode->next=Q1Ptr->Table[index].head;
    //head is not empty node, connect link between keynode and headnode
    if (Q1Ptr->Table[index].head != NULL) {
        Q1Ptr->Table[index].head->pre = keynode;
    }
    Q1Ptr->Table[index].head = keynode;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if (keynode==Q1Ptr->Table[index].head){
        Q1Ptr->Table[index].head=keynode->next;
    }else{
        keynode->pre->next=keynode->next;
    }
    //keynode is not last node
