...
int recursive_ks(int n, int *s, int *v, int C)
{
    if (dp[n][C]>0){
        return dp[n][C];
    }
    ...
    if (s[n]<=C){
        return max(recursive_ks(n-1,s,v,C-s[n])+v[n],recursive_ks(n-1,s,v,C));
    }else{
        return recursive_ks(n-1,s,v,C);
    }
